...
export function FilterChip({ active, onToggle, children, count }: Props) {
  return (
    <button
      ...
      aria-pressed={active}
      className={cn(
        "inline-flex min-h-9 items-center gap-1.5 rounded-md border px-3 py-1 text-xs font-semibold transition-[background-color,color,border-color,transform] duration-200 ease-out active:scale-[0.98]",
